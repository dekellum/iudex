...
* Add experimental support for concurrent (no discard) WorkPoller (in
  collaboration with iudex-core 1.4.0 VisitManager), enabled if
  do_reserve is set and do_discard is set false:
  * When enabled, Polled max_urls is adjusted by current order_count
    in this case.
  * An additional max_discard_ratio will also force a discard if
    order_count/max_urls passed this ratio.
