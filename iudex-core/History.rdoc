=== 1.4.0 (TBD)
* Work order reservation and experimental concurrent (no discard) work
  polling support:
...
* VisitManager.doWaitOnGeneration default true for default no reserve,
  mandatory discard case. Configuration suggestion: set false when
  also setting (iudex-da 1.4) WorkPoller.do_reserve.
