...
/**
 * Test for MurmurHashTest by Viliam Holub (public domain) with
...
{
    @Test
    public void test32BytesSeed()
    {
        for( int i = 0; i < input.length; ++i ) {
            int hash = MurmurHash32.hash( input[i], 0, input[i].length,
                                          0x71b4954d );
            if( hash != results32_seed[ i ] ) {
                fail( String.format( "Unexpected hash32 result for example " +
                                     "%d: 0x%08x instead of 0x%08x",
    ...
    @Test
    public void test64BytesSeed()
    {
        for( int i = 0; i < input.length; ++i ) {
            long hash = MurmurHash64.hash( input[i], 0, input[i].length,
                                           0x344d1f5c );
            assertTrue( String.format( "Unexpected hash64 result for example " +
                                       "%d: 0x%016x instead of 0x%016x",
    ...
    /**
     * Random input data of various lengths
     */
